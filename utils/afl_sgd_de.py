...
def discord_stale(conf, e, recieved_stamp):
    ...
    with open('DeFedAvg_'+conf["model_name"]+'_'+conf["type"]+'_staleness_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
...
def aggregrate_model(global_model, recieved_model, conf, e, freq_for_workers):
    ...
    import random
    active_recieved = random.sample(recieved_model, conf["k"])
    for item in active_recieved:
        recieved_model.remove(item)
    # active_recieved = recieved_model[:conf["k"]]
    # recieved_model = recieved_model[conf["k"]:]
    ...
    return global_model, freq_for_workers,recieved_model
...
def train_model(model, optimizer, data_loader, conf, seq):
    ...
    torch.save(gra_dict, "./defedavg/gradient_" + str(seq) + ".pt")
...
def main():
    ...
    for i in range(workers):
        ...
        worker_conf[i] = [resource, loader, 0, 0, "./defedavg/global_model_0.pt", val_loader_list]
    ...
    if conf["model_name"] == "resnet18":
        ...
        pass
    torch.save(global_model.state_dict(), "./defedavg/global_model_0.pt")
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        for client_seq_number in range(workers):
            ...
            if time_stamp == 0:
                ...
                if using_train_epoch != 0:
                    last_train_model = "./defedavg/global_model_"+str(using_train_epoch-1)+".pt"
                    if conf["model_name"] == "resnet18":
                    ...
                    last_g_gradient.load_state_dict(torch.load("./defedavg/gradient_" + str(client_seq_number) + ".pt"))
                ...
                gra =  "./defedavg/gradient_" + str(client_seq_number) + ".pt"
        ...
        if recieved_amount < conf["k"]:
            ...
            global_model, freq_for_workers, have_recieved_model = aggregrate_model(global_model, have_recieved_model, conf, global_epoch, freq_for_workers) 
            ...
            torch.save(global_model.state_dict(), "./defedavg/global_model_"+str(global_epoch)+".pt")

            # notice the newest global model to each client
            for client_seq_number in range(workers):
                worker_conf[client_seq_number][4] = './defedavg/global_model_'+str(global_epoch)+'.pt'
            ...
            this_time = time.time()
            with open('DeFedAvg_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
                # for row in ret:
            ...
            with open('DeFedAvg_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
