'''
Experiments on FedQS with similarity function as Euclidean Distance.
'''
...
def discord_stale(conf, e, recieved_stamp):
    ...
    with open('aflouroclidianAvg_'+conf["model_name"]+'_'+conf["type"]+'_staleness_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
...
def main():
    ...
    while global_epoch < conf["global_epochs"]:
            ...
            if time_stamp == 0:
                ...
                if using_train_epoch != 0:
                    ...
                    for name, value in last_g_model.state_dict().items():
                        ...
                        diff = (conv1_weight_a - conv1_weight_b).to(torch.float)
                        thetas.append(torch.linalg.norm(diff, ord=2))

                    if len(thetas) > 0:
                        theta_for_workers[client_seq_number] = sum(thetas)/len(thetas)
                    else:
                    ...
                    if theta_ths !=0 and speed_ths != 0:
                        if theta_for_workers[client_seq_number] > theta_ths:
                            if speed_for_workers[client_seq_number] > speed_ths:
                                ...
                                if standard_bias < 30:
                                    moment = 0.1 + max(0, min(20*(0.1*(theta_ths / theta_for_workers[client_seq_number]) - 0.1), 0.4))
                                else:
                                    feedback_for_workers[client_seq_number] = 1
                                    moment = 0
                        else:
                            ...
                            moment = 0.1 + min(20*(0.1*(theta_ths / theta_for_workers[client_seq_number]) - 0.1), 0.4)
        ...
        if recieved_amount < conf["k"]:
            ...
            this_time = time.time()
            with open('aflouroclidianAvg_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
            ...
            with open('aflouroclidianAvg_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
