'''
Experiments on FedQS with similarity function as Manhattan Distance.
'''
...
def discord_stale(conf, e, recieved_stamp):
    ...
    with open('aflourhamingAvg_'+conf["model_name"]+'_'+conf["type"]+'_staleness_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
...
def main():
    ...
    while global_epoch < conf["global_epochs"]:
            ...
            if time_stamp == 0:
                ...
                if using_train_epoch != 0:
                    ...
                    for name, value in last_g_model.state_dict().items():
                        ...
                        thetas.append(torch.linalg.norm(diff, p=1))
        ...
        if recieved_amount < conf["k"]:
            ...
            this_time = time.time()
            with open('aflourhamingAvg_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
            ...
            with open('aflourhamingAvg_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
