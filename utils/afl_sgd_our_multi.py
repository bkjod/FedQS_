'''
Experiments on FedQS within different client-resource settings.
'''
...
def discord_stale(conf, e, recieved_stamp):
    ...
    with open('aflour'+str(conf["no_models"])+'SGD_'+conf["model_name"]+'_'+conf["type"]+'_staleness_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
...
def main():
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        if recieved_amount < conf["k"]:
            ...
            this_time = time.time()
            with open('aflour'+str(conf["no_models"])+'SGD_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
            ...
            with open('aflour'+str(conf["no_models"])+'SGD_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
