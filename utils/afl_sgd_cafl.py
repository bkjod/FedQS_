...
def discord_stale(conf, e, recieved_stamp):
    ...
    with open('aflCAFLSGD_'+conf["model_name"]+'_'+conf["type"]+'_staleness_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([e, decay_stamp, sum(decay_stamp)])


# aggregrate the model
def aggregrate_model(global_model, recieved_model, conf, e, freq_for_workers):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    if conf["model_name"] == "resnet18":
        ...
        history_update = resnet_model.ResNet18(num=conf["CLASS_NUM"]).to(device)
        history_global = resnet_model.ResNet18(num=conf["CLASS_NUM"]).to(device)
        ...
        history_update = resnet_model.CNN(num=conf["CLASS_NUM"]).to(device)
        history_global = resnet_model.CNN(num=conf["CLASS_NUM"]).to(device)
    elif conf["model_name"] == "LSTM":
        gra = resnet_model.LSTM(num=conf["CLASS_NUM"]).to(device)
        history_update = resnet_model.LSTM(num=conf["CLASS_NUM"]).to(device)
        history_global = resnet_model.LSTM(num=conf["CLASS_NUM"]).to(device)
    else:
        pass
    ...
    for name, data in global_gradient.items():
        ...
        history_global.state_dict()[name].copy_(torch.zeros_like(data).to(device).float())
        if e == 0:
            history_update.state_dict()[name].copy_(torch.zeros_like(data).to(device).float())
    if e == 0:
        for i in range(conf["no_models"]):
            torch.save(history_update.state_dict(), "./cafl/history_local_"+str(i)+".pt")
    else:
        for name, data in history_global.state_dict().items():
            for i in range(conf["no_models"]): 
                history_update.load_state_dict(torch.load("./cafl/history_local_"+str(i)+".pt"))
                global_gradient[name] += history_update.state_dict()[name]/conf["no_models"]
            if data.type() != global_gradient[name].type():
                global_gradient[name] = torch.round(global_gradient[name]).to(torch.int64)
            else:
                pass
            history_global.state_dict()[name].add_(global_gradient[name])
    ...
    for name, data in global_model.state_dict().items():
        for gra_way in active_recieved:
            history_update.load_state_dict(torch.load("./cafl/history_local_"+str(gra_way[0])+".pt"))
            gra.load_state_dict(torch.load(gra_way[1]))
            gra_state = gra.state_dict()[name] - history_update.state_dict()[name]
            update_layer = history_global.state_dict()[name] + (gra_state / conf["k"]) 
            global_gradient[name] += update_layer*conf["local_lr"]*0.05
    ...
    for gra_way in active_recieved:
        gra.load_state_dict(torch.load(gra_way[1]))
        torch.save(gra.state_dict(), "./cafl/history_local_"+str(gra_way[0])+".pt")
...
def train_model(model, optimizer, data_loader, conf, seq):
    ...
    torch.save(gra_dict, "./cafl/gradient_" + str(seq) + ".pt")
...
def main():
    ...
    for i in range(workers):
        ...
        worker_conf[i] = [resource, loader, 0, 0, "./cafl/global_model_0.pt", val_loader_list]
    ...
    if conf["model_name"] == "resnet18":
        ...
        pass
    torch.save(global_model.state_dict(), "./cafl/global_model_0.pt")
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        for client_seq_number in range(workers):
            ...
            if time_stamp == 0:
                ...
                if using_train_epoch != 0:
                    last_train_model = "./cafl/global_model_"+str(using_train_epoch-1)+".pt"
                    if conf["model_name"] == "resnet18":
                    ...
                    last_g_gradient.load_state_dict(torch.load("./cafl/gradient_" + str(client_seq_number) + ".pt"))
                ...
                gra =  "./cafl/gradient_" + str(client_seq_number) + ".pt"
        ...
        if recieved_amount < conf["k"]:
            ...
            global_model, freq_for_workers = aggregrate_model(global_model, have_recieved_model, conf, global_epoch, freq_for_workers) 
            ...
            torch.save(global_model.state_dict(), "./cafl/global_model_"+str(global_epoch)+".pt")

            # notice the newest global model to each client
            for client_seq_number in range(workers):
                worker_conf[client_seq_number][4] = './cafl/global_model_'+str(global_epoch)+'.pt'
            ...
            this_time = time.time()
            with open('aflCAFLSGD_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
                # for row in ret:
            ...
            with open('aflCAFLSGD_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
