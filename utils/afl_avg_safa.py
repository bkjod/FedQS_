...
def discord_stale(conf, e, recieved_stamp):
    ...
    with open('aflsafaAvg_'+conf["model_name"]+'_'+conf["type"]+'_staleness_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([e, decay_stamp, sum(decay_stamp)])

def aggregate_model(global_model, recieved_model, conf, e, freq_for_workers,train_size_list):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    if conf["model_name"] == "resnet18":
        ...
        cache = resnet_model.ResNet18(num=conf["CLASS_NUM"]).to(device)
    elif conf["model_name"] == "vgg16":
        gra = resnet_model.Vgg16(num=conf["CLASS_NUM"]).to(device)
        cache = resnet_model.Vgg16(num=conf["CLASS_NUM"]).to(device)
    elif conf["model_name"] == "CNN":
        gra = resnet_model.CNN(num=conf["CLASS_NUM"]).to(device)
        cache = resnet_model.CNN(num=conf["CLASS_NUM"]).to(device)
    elif conf["model_name"] == "LSTM":
        gra = resnet_model.LSTM(num=conf["CLASS_NUM"]).to(device)
        cache = resnet_model.LSTM(num=conf["CLASS_NUM"]).to(device)
    else:
        pass
    P_idx = []
    P_list = []
    Q_list = []
    cache_type = [0]*conf["no_models"]
    for i in range(len(recieved_model)):
        if recieved_model[i][0] not in P_idx:
            P_idx.append(recieved_model[i][0])
            P_list.append(recieved_model[i])
        else:
            Q_list.append(recieved_model[i])
    active_recieved = []
    if len(P_list) < conf["k"]:
        active_recieved = P_list + Q_list[:conf["k"] - len(P_list)]
    else:
        active_recieved = P_list
    for gra_way in active_recieved:
        freq_for_workers[gra_way[0]] = e
    
    for i in range(len(freq_for_workers)):
        if i in P_idx:
            cache.load_state_dict(torch.load("./safa/gradient_" + str(i) + ".pt"))
            torch.save(cache.state_dict(), "./safa/cache_" + str(i) + ".pt")
        elif freq_for_workers[i] < e - 10:
            cache_type[i] = 1
            if e == 0:
                torch.save(global_gradient, "./safa/cache_" + str(i) + ".pt")
            else:
                cache.load_state_dict(torch.load("./safa/global_model_" + str(e-1) + ".pt"))
                torch.save(cache.state_dict(), "./safa/cache_" + str(i) + ".pt")
        else:
            if e == 0:
                torch.save(global_gradient, "./safa/cache_" + str(i) + ".pt")
    ...
    for i in range(conf["no_models"]):
        for name, data in global_model.state_dict().items():
            gra.load_state_dict(torch.load("./safa/cache_" + str(i) + ".pt"))
            gra_state = gra.state_dict()
            update_layer = (gra_state[name] * (train_size_list[i]/sum(train_size_list))) 
    ...
    for q_update in Q_list:
        cache.load_state_dict(torch.load(q_update[1]))
        torch.save(cache.state_dict(), "./safa/cache_" + str(q_update[0]) + ".pt")
...
def train_model(model, optimizer, data_loader, conf, seq):
    ...
    torch.save(model.state_dict(), "./safa/gradient_" + str(seq) + ".pt")
...
def main():
    ...
    for i in range(workers):
        ...
        worker_conf[i] = [resource, loader, 0, 0, "./safa/global_model_0.pt", val_loader_list]
    ...
    if conf["model_name"] == "resnet18":
        ...
        pass
    torch.save(global_model.state_dict(), "./safa/global_model_0.pt")
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        for client_seq_number in range(workers):
            ...
            if time_stamp == 0:
                ...
                gra =  "./safa/gradient_" + str(client_seq_number) + ".pt"
        ...
        if recieved_amount < conf["k"]:
            ...
            global_model, freq_for_workers = aggregate_model(global_model, have_recieved_model, conf, global_epoch, freq_for_workers,train_size_list) 
            ...
            have_recieved_model = []
            have_recieved_stamp = []
            torch.save(global_model.state_dict(), "./safa/global_model_"+str(global_epoch)+".pt")
            for client_seq_number in range(workers):
                worker_conf[client_seq_number][4] = './safa/global_model_'+str(global_epoch)+'.pt'
            ...
            this_time = time.time()
            with open('aflsafaAvg_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
            ...
            with open('aflsafaAvg_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
