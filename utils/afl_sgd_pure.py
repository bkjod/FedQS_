...
def aggregate_model(global_model, recieved_model, conf, e, freq_for_workers):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
...
def main():
    ...
    while global_epoch < conf["global_epochs"]:
        ...
        for client_seq_number in range(workers):
            ...
            if time_stamp == 0:
                ...
                if using_train_epoch != 0:
                    ...
                    last_g_gradient.load_state_dict(torch.load("./total/gradient_" + str(client_seq_number) + ".pt"))
        ...
        if recieved_amount < conf["k"]:
            ...
            global_model, freq_for_workers = aggregate_model(global_model, have_recieved_model, conf, global_epoch, freq_for_workers) 
            ...
            this_time = time.time()
            with open('aflpureSGD_'+conf["model_name"]+'_'+conf["type"]+'_acc_with'+'_alpha_'+str(conf["alpha"])+'_clip_'+str(conf["clip"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
            ...
            with open('aflpureSGD_'+conf["model_name"]+'_'+conf["type"]+'_size_with'+'_alpha_'+str(conf["alpha"])+'.csv', mode='a+', newline='') as file:
                writer = csv.writer(file)
